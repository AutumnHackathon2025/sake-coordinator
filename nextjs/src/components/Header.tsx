    ...
    <header className="fixed left-0 right-0 top-0 z-40 bg-primary py-3">
      <div className="relative flex items-center justify-center px-4">
        {enableHomeLink ? (
          ...
          <h1 className="text-body-lg font-medium text-text-light">{title}</h1>
        )}
        {rightAction ? (
          ...
          <Link
            href="/how-to-use"
            className="absolute right-4 flex items-center gap-1 text-text-light transition-opacity hover:opacity-80"
            aria-label="使い方"
          >
